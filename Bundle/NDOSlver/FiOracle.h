...
/*--------------------------------------------------------------------------*/
 ...
 public:
	virtual double final_feas(){return -1;}
/*--------------------------------------------------------------------------*/
/*---------------------------- PUBLIC TYPES --------------------------------*/
/*--------------------------------------------------------------------------*/
/** @name Public Types
...
/*--------------------------------------------------------------------------*/
/** Returns the Lipschitz constant. If 1 <= NrFi <= GetNrFi() [see above] the
    return value is about the component NrFi, while if NrFi > GetNrFi() then
    the return value is about the aggregated function.  Note that two cases
    are special:

     - the Lipschitz constant of aggregated function is less or equal to the
       sum of constants of its components;

     - NrFi == 0 is a special case since norm( b ) is the Lipschitz constant.

    A standard implementation is given for those oracles which do not provide
    a Lipschitz constant, maybe because Fi() is not Lipschitz continuous. */
...
/** @defgroup EasyComp Description of "easy" components of Fi()
    ...
    should in principle call these methods only once, or at least few times
    once and for all during the initialization stages. This means that these
    methods should not be a computational bottleneck, and no particular care
    should be required to implement them very efficiently. However, the NDO
    solver may call these methods a few times, and there are cases where
    repeated calls are clearly needed. The first is if the NDO solver uses
    variables generation techniques, which try to solve a NDO problem by
    restricting it to a (small) "active set" of the variables, minimizing the
    function in that subspace and possibly revising the active set. Such a
    NDO solver may acquire the information about different parts of the matrix
    A[ h ] in different moments, as it only works with a submatrix. Another is
    if the NDO solver discovers that one of the components which have a
    polyhedral description is changed (e.g., because the methods ChgFiV() and
    ChgSbG() of class NDOSolver are called, see NDOSlver.h); in this case, it
    may want to call these methods again to acquire the new description.
...
/** Returns a description of the matrix B[ wFi ] and of the vectors d[ wFi ],
    ...
    doubles; upon return, the vectors d[ wFi ] and e[ wFi ] have to be
    written there. Finally, cst, lbd and ubd must point to GetBNC()-vectors
    of doubles; upon return, the vectors c[ wFi ], l[ wFi ] and u[ wFi ] have
    to be written there.
...
/** Return a description of the submatrix of A[ wFi ] containing all the
    ...
    meaning of Abeg, Aind and Aval is analogous to that of Bbeg, Bind and
    Bval in GetBDesc() [see above]. */
...
/** The computation of the function Fi may be a costly task, e.g. involving
    ...
    giving me values affected by an relative error larger than Eps? And, in
