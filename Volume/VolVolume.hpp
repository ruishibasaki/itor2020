...
public: 
   ...
   VOL_primal(const VOL_primal& primal) :
      value(primal.value), x(primal.x), v(primal.v) {}
   ~VOL_primal() {}
...
public:
   ...
   // dual step
   double step(const double target, const double lambda, VOL_dual&trial,const VOL_dvector& v, double & t);
...
public:
   ...
   VOL_vh(const VOL_dvector& v, const VOL_dvector& vstar);
   ~VOL_vh(){}
};
...
public:
   ...
   */
   virtual int heuristics(const VOL_dvector& u, 
...
public: 
   ...
   /** Create a a <code>VOL_problem</code> object and read in the parameters
       from <code>filename</code>, <code>instance</code>. */
    VOL_problem(const char *filename, std::string instance);
   /** Destruct the object. */
   ~VOL_problem();
   //@}
...
private:
   ...
   /** Here we decide the value of alpha1 to be used in the convex
       ...
       @return alpha1
   */
   double power_heur(const VOL_vh& prod, const double & linerror, const double & sigma) const;
   //@}
};
